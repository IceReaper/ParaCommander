...
/// <summary>Gives a temporary sidewards gun when picked up.</summary>
public class GunSideItemComponent : StackingItemComponent<GunSideItemComponent>
{
        ...
        foreach (var weapon in this.armamentComponent.Weapons)
        {
            ...
            foreach (var weaponOffset in weapon.Offsets.Where(weaponOffset => !weaponOffset.Temporary))
            {
                weaponOffsets.Add(new WeaponOffset(
                    Vector2.Rotate(weaponOffset.Offset, MathHelper.Pi / 2),
                    Vector2.Rotate(weaponOffset.Direction, MathHelper.Pi / 2),
                    true));
    ...
    /// <inheritdoc />
    protected override void Stack(Entity entity, GameTime gameTime, GunSideItemComponent instance)
    {
